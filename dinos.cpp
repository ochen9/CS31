...
bool Player::shoot(int dir)
{
	...
	if (randInt(1, 3) > 1)  // miss with 2/3 probability
		...
		if(dir==UP){
			if(m_row==1)
				return false;
			for(int k = m_row-1; k>=1; k--)
			{
				if(m_valley->numDinosaursAt(k, m_col)>0)
				{
					m_valley->destroyDinosaur(k, m_col);
					return true;
				}
			}
		}
		if(dir==DOWN){
			if(m_row==m_valley->rows())
				return false;
			for(int k = m_row+1; k<=m_valley->rows(); k++)
			{
				if(m_valley->numDinosaursAt(k, m_col)>0)
				{
					m_valley->destroyDinosaur(k, m_col);
		...
		if(dir==LEFT){
			if(m_col==1)
				return false;
			for(int k = m_col-1; k>=1; k--)
			{
				if(m_valley->numDinosaursAt(m_row, k)>0)
				{
					m_valley->destroyDinosaur(m_row, k);
					return true;
				}
			}
		}
		if(dir==RIGHT){
			if(m_col==m_valley->cols())
				return false;
			for(int k = m_col-1; k<=m_valley->cols(); k++)
			{
				if(m_valley->numDinosaursAt(m_row, k)>0)
				{
					m_valley->destroyDinosaur(m_row, k);
...
bool Valley::destroyDinosaur(int r, int c)
{
	for (int i =0; i<m_nDinos; i++)
	{
		...
		{
			delete m_dinos[i];
			for(int k=i; k<m_nDinos-1; k++)
				m_dinos[k] = m_dinos[k+1];
			m_nDinos--;
			return true;
		}
	}
...
bool Valley::moveDinosaurs()
{
	for (int k = 0; k < m_nDinos; k++)
	{
		...
		// TODO:  DONE. Have the k-th dinosaur in the valley make one move.
